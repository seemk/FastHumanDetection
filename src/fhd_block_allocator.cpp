...
fhd_block_allocator* fhd_block_allocator_create(int block_size, int num_blocks) {
  fhd_block_allocator* block_alloc = (fhd_block_allocator*)calloc(1, sizeof(fhd_block_allocator));
  ...
  block_alloc->full_slots = (fhd_alloc_slot*)calloc(num_blocks, sizeof(fhd_alloc_slot));
...
void* fhd_block_allocator_acquire(fhd_block_allocator* block_alloc) {
  ...
  const int free_idx = block_alloc->free_indices[block_alloc->free_indices_len - 1];
...
void fhd_block_allocator_clear(fhd_block_allocator* block_alloc) {
  for (int i = 0; i < block_alloc->full_slots_len; i++) {
    block_alloc->free_indices[block_alloc->free_indices_len++] = block_alloc->full_slots[i].idx;
  }
